...
export default function Group() {
  ...
  useEffect(() => {
    setAcademikGroup([])
    getGroups(`${academic_group}?page_size=${pageSize}&page=${page}&search=${SearchText}&direction=${user['role'] != 'dekan' ? DirectionID : ""}&year=${AcademekYear}`, (response) => {
  ...
  useEffect(() => {
    user['role'] != 'dekan' && getDirections(`${directions}?page_size=100`, (response) => {
      // setDirections(response.results)
      setDirectionSelect(response?.results?.[0].id)
  ...
  const LangList = useMemo(() => {
    setLangSelect(language[0].value)
      ...
      >
        ...
        <BoxHeader>
          <InputsWrapper>
            ...
            {user['role'] != 'dekan' && <AutocompleteJames width={'200px'} selectOptions={Directions} chageValueFunction={val => setDirectionID(val)} label={"Yo'nalish"} />}
          ...
          <th>
...

            {
              ...
              >
                ...
                <Link to={'personalplan'} state={{ id: elem.direction, group: elem.id, year: elem.year, study_type: elem.study_type }}>
                  <IconButton style={{ margin: "10px 0 0 20px", width: '150px' }}>
                    Shaxsiy reja
                  </IconButton>
                </Link>
              </>
            ...
            <Link to={'students'} state={elem.id}>
              <IconButton style={{ margin: "10px 10px", width: '80px' }}>
                Talabalar
              </IconButton>
            </Link>

...
export default function Faculties() {
  ...
  }, [addElement, pageSize, page])

  const facultytype = useMemo(() => {
    return faculty_type.map(element => {
      return {
        value: element.value,
        name: element.uz
      }
  ...
  const createFacultyHandler = (_) => {
    fakultyCreate(fakulty, requestData, (response) => {
      handleClose()
      if (response.data?.id) {
        setOpenAlert(true)
        serChanged(true)
  ...
  return (
    <>
      ...
      >
        ...
        <BoxFooter>
          <BoxFooterText>{`Jami ${allCount} ta, ${pageSize * (page - 1) + 1} dan ${pageSize * (page - 1) + facultyList.length} gachasi ko'rsatilmoqda`}</BoxFooterText>
      ...
      <Snackbar open={openAlert} anchorOrigin={changed ? anchorOrigin1 : anchorOrigin2} autoHideDuration={6000} onClose={handleCloseAlert}>
        <Alert onClose={handleCloseAlert} severity={changed ? "success" : "error"} sx={{ width: '100%' }}>
          {changed ? <span>Fakultet yaratildi!</span> : <span>Fakultet yaratishda xatolik yuzaga keldi.</span>}
        </Alert>
      </Snackbar>
    </>
  )
          ...
          <tr>
            ...
            <th>
              ...
              <Button
                ...
                onClick={(_) => {deleteElementFaculty(elem.id)}}
                startIcon={<svg width="17" height="16" viewBox="0 0 17 16" fill="none" xmlns="http://www.w3.org/2000/svg">

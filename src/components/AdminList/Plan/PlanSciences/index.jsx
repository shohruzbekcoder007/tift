...
export default function PlanSciences() {
  ...
  useEffect(() => {
    ...
    getAcademic_ShortScience(`${science}short/?degree=${state.degree}&direction=${state.direction}&academic_year=${state.year}&study_type=${state.study_type}&semester=${state.semester}`, response => {
      let list = []
  ...
  const handleDelete = (_) => {
    deleteAcademic_Science(`${academic_plan_science}${DeletedID}`, response => {
      setDeleted(!deleted)
    }, error => {
      console.log(error)
    })
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <AlertDialog
                          ...
                          alertText={"Ushbu Fanni haqiqatdan ham o'chirmoqchimisiz?"}
                        />
                      </tr>
                    )
                  })
          ...
          <ModalBoxScience>
            ...
            <ModalBody>
              ...
              <ModalSelectWrapper>
               ...
               {SciencesList.length > 0 && 
                <AutocompleteJames
                  chageValueFunction={val => setSciencesID(val)}
                  selectedOptionP={parseInt(SciencesList[0]?.value)}
